...
public class PluginMappingControlFactory : MappingControlFactory
{
    ...
    public override T CreateInstance<T>() => (T)this.pluginHost.CreateControl(this.controlTypeName);

    /// <summary>
    /// Since we can't get the Type in the other appdomain, we return the host/contract class it derives from instead.
    /// </summary>
    /// <returns></returns>
    public override Type ToType() => throw new InvalidOperationException("Cannot get Type in other appdomain.");

    public override string GetTypeName() => this.controlTypeName;
}
